...
### root specific
if [[ $(id -u) == 0 ]]; then
	FILE="$HOME/scripts/etc/udev/rules.d/99-yubikeys.rules"
	test -f $FILE && ln -sf $FILE /etc/udev/rules.d/99-yubikeys.rules
	FILE="$HOME/scripts/etc/udev/rules.d/51-android.rules"
	test -f $FILE && ln -sf $FILE /etc/udev/rules.d/51-android.rules
	#
	echo "Creating symlink for sudoers"
	if [ -d /etc/sudoers.d ]; then
	  ln $HOME/scripts/etc/sudoers.d/global_sudo /etc/sudoers.d/globalsudo
	...
	ln -sf $HOME/scripts/profiles/aptitude-config $HOME/.aptitude/config
fi
...
#echo "  Legacy network scripts"
#if [ ! -d $HOME/git/network_scripts_legacy ] ; then
  #git clone git@bitbucket.org:bambam82/network_scripts_legacy.git $HOME/git/network_scripts_legacy
#fi
